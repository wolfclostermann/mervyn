...
| ----------------- | ------------ |
| **Reminder** | Stores a reminder in `redb`. Understands a time written as `5pm`, `6 am`, `7p.m.`, `17:00` or `5:30am`, and an ISO date `YYYY-MM-DD`, `today` or `tomorrow`. With no time it defaults to 09:00; with nothing recognised at all, about **tomorrow** at 09:00 UTC. Weekday names are understood too — full names anywhere (`football saturday`), and short forms when something in front of them says a day is coming (`on Weds`, `next Sat`), so that "I sat in the garden" stays prose. A weekday naming today means the *next* one unless a time later today is given. `next Weds` currently means the same as `Weds` — the nearest one — rather than the week after. Due reminders are **posted to `TELEGRAM_CHAT_ID`** on a schedule (see below). |
